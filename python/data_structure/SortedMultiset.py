...
class SortedMultiset(Generic[T]):
    ...
    def lower_bound_cursor(self, x: T) -> "SortedMultisetCursor[T]":
        """Return the boundary just before the first element >= x.
        Any modification to the multiset invalidates existing cursors.
        """
        for b, a in enumerate(self.a):
            if a[-1] >= x:
                return SortedMultisetCursor(self, b, bisect_left(a, x))
        return SortedMultisetCursor(self, len(self.a), 0)

    def upper_bound_cursor(self, x: T) -> "SortedMultisetCursor[T]":
        """Return the boundary just before the first element > x.
        Any modification to the multiset invalidates existing cursors.
        """
        for b, a in enumerate(self.a):
            if a[-1] > x:
                return SortedMultisetCursor(self, b, bisect_right(a, x))
        return SortedMultisetCursor(self, len(self.a), 0)

    def erase(self, first: "SortedMultisetCursor[T]", last: "SortedMultisetCursor[T]") -> int:
        ...
        if first.s is not self or last.s is not self:
            raise ValueError("cursor belongs to another SortedMultiset")
...
class SortedMultisetCursor(Generic[T]):
    """A cursor at a boundary between two elements of a SortedMultiset."""

    __slots__ = ("s", "b", "i")

    def __init__(self, s: SortedMultiset[T], b: int, i: int) -> None:
        """多重集合とバケット番号・バケット内添字を参照するカーソルを作る。"""
        self.s = s
        self.b = b
        self.i = i

    def copy(self) -> "SortedMultisetCursor[T]":
        """同じ要素を持つ浅いコピーを返す。"""
        return SortedMultisetCursor(self.s, self.b, self.i)
